...
export default function QuizPlayer({ quiz }: { quiz: Quiz }) {
  ...
  const [finished,setFinished]=useState(false); const [shareText,setShareText]=useState("Share result"); const [bookmarked,setBookmarked]=useState(()=>{if(typeof window==="undefined")return false;try{const saved=JSON.parse(localStorage.getItem("mindrailo-bookmarks")??"[]") as string[];return saved.includes(quiz.slug);}catch{return false;}});
  ...
  const share=async()=>{const url=window.location.href; try{if(navigator.share){await navigator.share({title:quiz.title,text:`My Mindrailo result: ${result?.title}`,url});setShareText("Shared");}else{await navigator.clipboard.writeText(`${quiz.title} — My result: ${result?.title}. ${url}`);setShareText("Result copied");}trackEvent("share_result",{quiz_slug:quiz.slug,result_id:result?.id??"unknown"});}catch{setShareText("Share result")}};
  const toggleBookmark=()=>{const saved=JSON.parse(localStorage.getItem("mindrailo-bookmarks")??"[]") as string[];const next=saved.includes(quiz.slug)?saved.filter(x=>x!==quiz.slug):[quiz.slug,...saved];localStorage.setItem("mindrailo-bookmarks",JSON.stringify(next));setBookmarked(next.includes(quiz.slug));trackEvent("bookmark_quiz",{quiz_slug:quiz.slug,saved:next.includes(quiz.slug)});};
